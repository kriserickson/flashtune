...
const getPlayableYears = (opts: { genres: Genre[]; timeSpan: TimeSpan }): number[] => [...new Set(getPlayablePool({ genres: opts.genres, timeRange: { end: opts.timeSpan, start: opts.timeSpan } }).map((song) => song.year))].sort(
		(a, b) => a - b
	);
