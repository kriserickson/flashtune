...
const getSuccessVariants = (year: number): readonly string[] => {
	for (const variant of SUCCESS_VARIANTS) {
		if (year < variant.threshold) {return variant.options;}
	}
...
const VERDICT_VARIANTS: { threshold: number; options: readonly ResultCopy[] }[] = [
	{
		...
		options: [
			...
			{ comment: 'That was crate-digger clairvoyance from start to finish.', headline: 'Flawless run.' },
	...
	},
	{
		...
		options: [
			...
			{ comment: 'The timeline only slipped away a couple of times.', headline: 'You had the groove.' }
		]
	},
	{
		threshold: RATIO_GOOD,
		options: [
			{ comment: 'More hits than misses, and some very clean placements.', headline: 'Solid round.' },
	...
	},
	{
		...
		options: [
			{ comment: 'Plenty went right, even if the round got tricky.', headline: 'Right in the mix.' },
			...
			{ comment: 'A few sharper reads and this turns into a big score.', headline: 'Promising run.' }
		]
	},
	{
		threshold: RATIO_WARM,
		options: [
			{ comment: 'The feel for the timeline is there, just not consistently yet.', headline: 'Getting warmer.' },
			{ comment: 'Some placements were excellent, some were chaos.', headline: 'Moments of brilliance.' },
			{ comment: 'A few more clean reads and this score jumps fast.', headline: 'You were around it.' },
...
const getVerdictVariants = (ratio: number): readonly ResultCopy[] => {
	for (const variant of VERDICT_VARIANTS) {
		if (ratio >= variant.threshold) {return variant.options;}
	}
