...
export function nextGenreSelection(
	...
	if (next.has(genre)) {
		if (next.size === 1) {return { allGenres, selected };}
		next.delete(genre);
	} else {
		next.add(genre);
...
export function getAvailablePlacementCounts(
	...
	return placementCounts.filter((count) => availableCount >= count + PLACEMENT_OFFSET) as PlacementCount[];
}

export function selectPlacementCount(
	next: PlacementCount,
	availableCount: number
): boolean {
...
export function normalizePlacementCount(
	...
	if (availablePlacementCounts.includes(placementCount)) {return placementCount;}
